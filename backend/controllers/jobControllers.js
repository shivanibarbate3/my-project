...
const addEditjob = async (req, res) => {
  ...
  try {
    ...
    if (existingauth.id == undefined) {
      ...
      const { id, title, location, no_vac, date_join, date_end, det } = req.body;
      ...
      try {
        if (id == 0 || id == null) {
          const locationrAlreadyExists = await checkRecordExists("careers_new", "title", title);
          if (locationrAlreadyExists) {
            res.status(200).json({ status: false, message: "Job already exists" });
          } else {
            await insertRecord("careers_new", slider);
            res.status(201).json({ status: true, message: "Job created successfully!" });
          }
        } else {
          const locationrAlreadyExists = await checkAllRecordExists("careers_new", "title", title, id);
          ...
          if (locationrAlreadyExists) {
            ...
            await UpdateRecord("careers_new", slider, id);
            res.status(201).json({ status: true, message: "Job updated successfully!" });
          }
        }
    ...
    if (existingauth.id == undefined) {
      ...
      try {
        ...
        if (search != '') {
          ...
          const existingUserList = await Recordlistseach("careers_new", search_by, search, "id", perPage, pageOffset);
          if (existingUserList) {

            const existingUserAllList = await Recordalllistsearch("careers_new", search_by, search, "id");
          ...
          const existingUserList = await Recordlist("careers_new", null, null, "id", perPage, pageOffset);
          if (existingUserList) {

            const existingUserAllList = await Recordalllist("careers_new", null, null, "id");
