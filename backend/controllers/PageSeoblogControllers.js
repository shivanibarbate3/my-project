...
const addEditPageSeoblog = async (req, res) => {
  ...
  try {
    ...
    if (existingauth.id == undefined) {
      ...
      try {
        if (id == 0 || id == null) {
          const meta_titlerAlreadyExists = await checkRecordExists("blogpageseotag", "page_name", page_name);
          if (meta_titlerAlreadyExists) {
            res.status(200).json({ status: false, message: "Blog Page Seo already exists" });
          } else {
            await insertRecord("blogpageseotag", slider);
            res.status(201).json({ status: true, message: "Blog Page Seo created successfully!" });
          }
        } else {
          const meta_titlerAlreadyExists = await checkAllRecordExists("blogpageseotag", "page_name", page_name, id);
          ...
          if (meta_titlerAlreadyExists) {
            ...
            await UpdateRecord("blogpageseotag", slider, id);
            res.status(201).json({ status: true, message: "Blog Page Seo updated successfully!" });
          }
        }
...
const getPageSeoblog = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if (search_by == 'meta_title') {
        ...
        const existingUserList = await Recordlistseachjoin("blogpageseotag", search_by, search,"id",perPage,pageOffset,'blog','page_name','title');
        if (existingUserList) {
      
          const existingUserAllList = await Recordalllistsearchjoin("blogpageseotag", search_by, search,"id",'blog','page_name','title');
        ...
        const existingUserList = await Recordlistjoin("blogpageseotag", null, null,"id",perPage,pageOffset,'blog','page_name','title');
        if (existingUserList) {
      
          const existingUserAllList = await Recordalllistjoin("blogpageseotag", null, null,"id",'blog','page_name','title');
