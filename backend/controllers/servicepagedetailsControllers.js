...
const addEditServicepagedetails = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      if(id==0 || id==null)  {
        const hometilerAlreadyExists = await checkRecordExists("servicepagedetails_new", "page_title", page_title);
        if (hometilerAlreadyExists) {
          res.status(200).json({ status: false, message: "Page Detailsalready exists" });
        } else {
          await insertRecord("servicepagedetails_new", hometile);
          res.status(201).json({ status: true, message: "Page Details created successfully!" });
        }
      }else{
        const hometilerAlreadyExists = await checkAllRecordExists("servicepagedetails_new", "page_title", page_title,id);
        ...
        if (hometilerAlreadyExists) {
          ...
          await UpdateRecord("servicepagedetails_new", hometile,id);
...
const getServicepagedetails = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    try {
     ...
     const existingUserList = await Recordlist("servicepagedetails_new", null, null,"id",perPage,pageOffset);
 
     if (existingUserList) {
       const existingUserAllList = await Recordalllist("servicepagedetails_new", null, null,"id");
