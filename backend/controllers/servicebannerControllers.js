...
const addEditServiceBanner = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    if(id!=0){
      ...
      const locationrAlreadyExistsfornew = await checkRowExists("servicebanner_new", id);
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("servicebanner_new", "banner_title", banner_title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Banner already exists" });
        } else {
          await insertRecord("servicebanner_new", servicebanner_new);
          res.status(201).json({ status: true, message: "Banner created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("servicebanner_new", "banner_title", banner_title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("servicebanner_new", servicebanner_new,id);
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if(search!=''){
        ...
        const existingUserList = await Recordlistseach("servicebanner_new", search_by, search,"id",perPage,pageOffset);
       
        if (existingUserList) {
          const existingUserAllList = await Recordalllistsearch("servicebanner_new", search_by, search,"id");
        ...
        const existingUserList = await Recordlist("servicebanner_new", null, null,"id",perPage,pageOffset);
       
           if (existingUserList) {
             const existingUserAllList = await Recordalllist("servicebanner_new", null, null,"id");
