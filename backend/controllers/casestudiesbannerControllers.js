...
const addEditCasestudiesbanner = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    if(id!=0){
      ...
      const locationrAlreadyExistsfornew = await checkRowExists("casestudiesbanner_new", id);
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("casestudiesbanner_new", "banner_title", banner_title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Banner already exists" });
        } else {
          await insertRecord("casestudiesbanner_new", casestudiesbanner_new);
          res.status(201).json({ status: true, message: "Banner created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("casestudiesbanner_new", "banner_title", banner_title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("casestudiesbanner_new", casestudiesbanner_new,id);
...
const getCasestudiesbanner = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if(search!=''){
        ...
        const existingUserList = await Recordlistseach("casestudiesbanner_new", search_by, search,"id",perPage,pageOffset);
       
        if (existingUserList) {
          const existingUserAllList = await Recordalllistsearch("casestudiesbanner_new", search_by, search,"id");
        ...
        const existingUserList = await Recordlist("casestudiesbanner_new", null, null,"id",perPage,pageOffset);
       
           if (existingUserList) {
             const existingUserAllList = await Recordalllist("casestudiesbanner_new", null, null,"id");
...
module.exports = {
  addEditCasestudiesbanner,
  getCasestudiesbanner,
