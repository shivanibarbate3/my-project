...
const addEditVision = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    if(id!=0){
      ...
      const locationrAlreadyExistsfornew = await checkRowExists("homevisision_new", id);
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("homevisision_new", "title", title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Vision already exists" });
        } else {
          await insertRecord("homevisision_new", homevisision);
          res.status(201).json({ status: true, message: "Vision created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("homevisision_new", "title", title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("homevisision_new", homevisision,id);
          res.status(201).json({ status: true, message: "Vision updated successfully!" });
        }
      }
...
const getVision = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    try {
     ...
     const existingUserList = await Recordlist("homevisision_new", null, null,"id",perPage,pageOffset);
 
     if (existingUserList) {
       const existingUserAllList = await Recordalllist("homevisision_new", null, null,"id");
