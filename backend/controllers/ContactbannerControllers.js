...
const addEditContactbanner = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    if(id!=0){
      ...
      const locationrAlreadyExistsfornew = await checkRowExists("contactsbanner_new", id);
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("contactsbanner_new", "banner_title", banner_title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Banner already exists" });
        } else {
          await insertRecord("contactsbanner_new", contactsbanner_new);
          res.status(201).json({ status: true, message: "Banner created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("contactsbanner_new", "banner_title", banner_title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("contactsbanner_new", contactsbanner_new,id);
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if(search!=''){
        ...
        const existingUserList = await Recordlistseach("contactsbanner_new", search_by, search,"id",perPage,pageOffset);
       
        if (existingUserList) {
          const existingUserAllList = await Recordalllistsearch("contactsbanner_new", search_by, search,"id");
        ...
        const existingUserList = await Recordlist("contactsbanner_new", null, null,"id",perPage,pageOffset);
       
           if (existingUserList) {
             const existingUserAllList = await Recordalllist("contactsbanner_new", null, null,"id");
