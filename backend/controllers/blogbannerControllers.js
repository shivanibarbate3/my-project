...
const addEditBlogbanner = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    if(id!=0){
      ...
      const locationrAlreadyExistsfornew = await checkRowExists("blogbanner", id);
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("blogbanner", "banner_title", banner_title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Banner already exists" });
        } else {
          await insertRecord("blogbanner", blogbanner);
          res.status(201).json({ status: true, message: "Banner created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("blogbanner", "banner_title", banner_title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("blogbanner", blogbanner,id);
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if(search!=''){
        ...
        const existingUserList = await Recordlistseach("blogbanner", search_by, search,"id",perPage,pageOffset);
       
        if (existingUserList) {
          const existingUserAllList = await Recordalllistsearch("blogbanner", search_by, search,"id");
        ...
        const existingUserList = await Recordlist("blogbanner", null, null,"id",perPage,pageOffset);
       
           if (existingUserList) {
             const existingUserAllList = await Recordalllist("blogbanner", null, null,"id");
