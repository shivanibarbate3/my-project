...
const addEditCareerbanner = async (req, res) => {
...
if(results.length>0 && index!=-1){
    ...
    if(id!=0){
      ...
      const locationrAlreadyExistsfornew = await checkRowExists("careerbanner_new", id);
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("careerbanner_new", "banner_title", banner_title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Banner already exists" });
        } else {
          await insertRecord("careerbanner_new", careerbanner_new);
          res.status(201).json({ status: true, message: "Banner created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("careerbanner_new", "banner_title", banner_title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("careerbanner_new", careerbanner_new,id);
...
const getCareerbanner = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if(search!=''){
        ...
        const existingUserList = await Recordlistseach("careerbanner_new", search_by, search,"id",perPage,pageOffset);
       
        if (existingUserList) {
          const existingUserAllList = await Recordalllistsearch("careerbanner_new", search_by, search,"id");
        ...
        const existingUserList = await Recordlist("careerbanner_new", null, null,"id",perPage,pageOffset);
       
           if (existingUserList) {
             const existingUserAllList = await Recordalllist("careerbanner_new", null, null,"id");
