...
const addEditPageSeoproject = async (req, res) => {
  ...
  try {
    ...
    if (existingauth.id == undefined) {
      ...
      try {
        if (id == 0 || id == null) {
          const meta_titlerAlreadyExists = await checkRecordExists("projectpageseotag", "page_name", page_name);
          if (meta_titlerAlreadyExists) {
            res.status(200).json({ status: false, message: "Project Page Seo already exists" });
          } else {
            await insertRecord("projectpageseotag", slider);
            res.status(201).json({ status: true, message: "Project Page Seo created successfully!" });
          }
        } else {
          const meta_titlerAlreadyExists = await checkAllRecordExists("projectpageseotag", "page_name", page_name, id);
          ...
          if (meta_titlerAlreadyExists) {
            ...
            await UpdateRecord("projectpageseotag", slider, id);
            res.status(201).json({ status: true, message: "Project Page Seo updated successfully!" });
          }
        }
...
const getPageSeoproject = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if (search_by == 'meta_title') {
        ...
        const existingUserList = await Recordlistseachjoin("projectpageseotag", search_by, search,"id",perPage,pageOffset,'projects_new','page_name','title');
        if (existingUserList) {
      
          const existingUserAllList = await Recordalllistsearchjoin("projectpageseotag", search_by, search,"id",'projects_new','page_name','title');
        ...
        const existingUserList = await Recordlistjoin("projectpageseotag", null, null,"id",perPage,pageOffset,'projects_new','page_name','title');
        if (existingUserList) {
      
          const existingUserAllList = await Recordalllistjoin("projectpageseotag", null, null,"id",'projects_new','page_name','title');
