...
const addEditAboutBanner = async (req, res) => {
...
if(results.length>0 && index!=-1){
    ...
    if(id!=0){
      ...
      const locationrAlreadyExistsfornew = await checkRowExists("aboutbanner_new", id);
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("aboutbanner_new", "banner_title", banner_title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Banner already exists" });
        } else {
          await insertRecord("aboutbanner_new", aboutbanner_new);
          res.status(201).json({ status: true, message: "Banner created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("aboutbanner_new", "banner_title", banner_title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("aboutbanner_new", aboutbanner_new,id);
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if(search!=''){
        ...
        const existingUserList = await Recordlistseach("aboutbanner_new", search_by, search,"id",perPage,pageOffset);
       
        if (existingUserList) {
          const existingUserAllList = await Recordalllistsearch("aboutbanner_new", search_by, search,"id");
        ...
        const existingUserList = await Recordlist("aboutbanner_new", null, null,"id",perPage,pageOffset);
       
           if (existingUserList) {
             const existingUserAllList = await Recordalllist("aboutbanner_new", null, null,"id");
