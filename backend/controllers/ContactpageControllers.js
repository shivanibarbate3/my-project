...
const addEditContactpage = async (req, res) => {
  ...
  try {
    ...
    if (existingauth.id == undefined) {
      ...
      const { id, address, email, phone, linkdin_link } = req.body;
      ...
      try {
        if (id == 0 || id == null) {
          const locationrAlreadyExists = await checkRecordExists("contactpagedetails_new", "id", id);
          if (locationrAlreadyExists) {
            res.status(200).json({ status: false, message: " Page Details already exists" });
          } else {
            await insertRecord("contactpagedetails_new", contactpagedetails_new);
            res.status(201).json({ status: true, message: " Page Details created successfully!" });
          }
        } else {
          const locationrAlreadyExists = await checkAllRecordExists("contactpagedetails_new", "id", id, id);
          ...
          if (locationrAlreadyExists) {
            ...
            await UpdateRecord("contactpagedetails_new", contactpagedetails_new, id);
    ...
    if (existingauth.id == undefined) {
      ...
      try {
        ...
        if (search != '') {
          ...
          const existingUserList = await Recordlistseach("contactpagedetails_new", search_by, search, "id", perPage, pageOffset);

          if (existingUserList) {
            const existingUserAllList = await Recordalllistsearch("contactpagedetails_new", search_by, search, "id");
            ...
            existingUserList.forEach((element) => {
              let myObject = {
                id: element.id, address: element.address,
                phone: element.phone,
          ...
          const existingUserList = await Recordlist("contactpagedetails_new", null, null, "id", perPage, pageOffset);

          if (existingUserList) {
            const existingUserAllList = await Recordalllist("contactpagedetails_new", null, null, "id");
