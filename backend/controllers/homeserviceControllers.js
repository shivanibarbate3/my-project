...
const addEditHomeService = async (req, res) => {
  ...
  if (existingauth.id==undefined) {
    ...
    const { id, title, short_des,subtitle } = req.body;
    ...
    try {
      if(id==0 || id==null)  {
        const locationrAlreadyExists = await checkRecordExists("homeservies_new", "title", title);
        if (locationrAlreadyExists) {
          res.status(200).json({ status: false, message: "Home Service already exists" });
        } else {
          await insertRecord("homeservies_new", homeservies_new);
          res.status(201).json({ status: true, message: "Home Service created successfully!" });
        }
      }else{
        const locationrAlreadyExists = await checkAllRecordExists("homeservies_new", "title", title,id);
        ...
        if (locationrAlreadyExists) {
          ...
          await UpdateRecord("homeservies_new", homeservies_new,id);
          res.status(201).json({ status: true, message: "Home Service updated successfully!" });
        }
      }
  ...
  if (existingauth.id==undefined) {
    ...
    try {
      ...
      if(search!=''){
        ...
        const existingUserList = await Recordlistseach("homeservies_new", search_by, search,"id",perPage,pageOffset);
       
        if (existingUserList) {
          const existingUserAllList = await Recordalllistsearch("homeservies_new", search_by, search,"id");
        ...
        const existingUserList = await Recordlist("homeservies_new", null, null,"id",perPage,pageOffset);
       
           if (existingUserList) {
             const existingUserAllList = await Recordalllist("homeservies_new", null, null,"id");
