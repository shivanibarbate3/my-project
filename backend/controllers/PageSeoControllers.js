...
const addEditPageSeo = async (req, res) => {
  ...
  try {
    ...
    if (existingauth.id == undefined) {
      ...
      try {
        if (id == 0 || id == null) {
          const meta_titlerAlreadyExists = await checkRecordExists("pageseotag", "page_name", page_name);
          if (meta_titlerAlreadyExists) {
            res.status(200).json({ status: false, message: "Page Seo already exists" });
          } else {
            await insertRecord("pageseotag", slider);
            res.status(201).json({ status: true, message: "Page Seo created successfully!" });
          }
        } else {
          const meta_titlerAlreadyExists = await checkAllRecordExists("pageseotag", "page_name", page_name, id);
          ...
          if (meta_titlerAlreadyExists) {
            ...
            await UpdateRecord("pageseotag", slider, id);
            res.status(201).json({ status: true, message: "Page Seo updated successfully!" });
          }
        }
    ...
    if (existingauth.id == undefined) {
      ...
      try {
        ...
        if (search != '') {
          ...
          const existingUserList = await Recordlistseach("pageseotag", search_by, search, "id", perPage, pageOffset);
          if (existingUserList) {

            const existingUserAllList = await Recordalllistsearch("pageseotag", search_by, search, "id");
          ...
          const existingUserList = await Recordlist("pageseotag", null, null, "id", perPage, pageOffset);
          if (existingUserList) {

            const existingUserAllList = await Recordalllist("pageseotag", null, null, "id");


            const array = [];
            existingUserList.forEach((element) => {
              let myObject = {
                id: element.id, page_name: element.page_name,
                meta_title: element.meta_title,
